...
markers:function(b,a,d){var c="",e=0,m=0,h=[],k=[];a=a?a:this.options;q=p=0;if(!d||0===h.length){if(g!==a.marker&&0<a.marker.length)for(c in a.marker)f(a.marker,c)&&f(a.marker[c],"addr")&&("object"===typeof a.marker[c].addr&&2===a.marker[c].addr.length?this.markerDirect(b,a.marker[c],a):"string"===typeof a.marker[c].addr&&this.markerByGeocoder(b,a.marker[c],a))}else if("modify"===d&&"modify"===d)for(h=this._markers,k=this._labels,e=0;e<a.marker.length;e+=1)if(g!==a.marker[e].id){for(m=0;m<h.length;m+=
1)a.marker[e].id===h[m].id&&g!==a.marker[e].addr&&(h[m].setPosition(new google.maps.LatLng(a.marker[e].addr[0],a.marker[e].addr[1])),"function"===typeof h[e].infoWindow.setContent&&h[m].infoWindow.setContent(a.marker[e].text));for(m=0;m<k.length;m+=1)a.marker[e].id===k[m].id&&(f(a.marker[e],"label")&&(k[m].text=a.marker[e].label),k[m].draw())}else"object"===typeof a.marker[e].addr&&2===a.marker[e].addr.length?this.markerDirect(b,a.marker[e]):"string"===typeof a.marker[e].addr&&this.markerByGeocoder(b,
a.marker[e]);if(f(a,"markerCluster")&&!0===a.markerCluster&&"function"===typeof MarkerClusterer)return new MarkerClusterer(b,this._markers)},drawPolyline:function(b,a){var d={},d="",c={},e=[];a=a?a:this.options;if(g!==a.polyline&&g!==a.polyline.coords){for(d in a.polyline.coords)f(a.polyline.coords,d)&&(c=a.polyline.coords,g!==c[d]&&e.push(new google.maps.LatLng(c[d][0],c[d][1])));d=new google.maps.Polyline({path:e,strokeColor:a.polyline.color||"#FF0000",strokeOpacity:1,strokeWeight:a.polyline.width||
...
radius:c.radius||10,zIndex:100}),this._circles.push(c),l.isFunction(a.circle[d].click)&&google.maps.event.addListener(c,"click",a.circle[d].click))},overlay:function(){var b=this.map,a=this.options;this.kml(b,a);this.direction(b,a);this.markers(b,a);this.drawPolyline(b,a);this.drawPolygon(b,a);this.drawCircle(b,a)},markerIcon:function(b){var a={};if(f(b,"icon")){if("string"===typeof b.icon)return b.icon;f(b.icon,"url")&&(a.url=b.icon.url);f(b.icon,"size")&&g!==b.icon.size[0]&&g!==b.icon.size[1]&&
(a.scaledSize=new google.maps.Size(b.icon.size[0],b.icon.size[1]));f(b.icon,"anchor")&&g!==b.icon.anchor[0]&&g!==b.icon.anchor[1]&&(a.anchor=new google.maps.Point(b.icon.anchor[0],b.icon.anchor[1]))}return a},markerDirect:function(b,a){var d={},c={},c={},c=f(a,"id")?a.id:"",d=f(a,"title")?a.title.toString().replace(/<([^>]+)>/g,""):"",e=f(a,"text")?a.text.toString():"",e={map:b,position:new google.maps.LatLng(a.addr[0],a.addr[1]),infoWindow:new google.maps.InfoWindow({content:e}),animation:null,id:c},
m=this.markerIcon(a);d&&(e.title=d);p+=1;if("string"===typeof m||f(m,"url"))e.icon=m;f(a.animation)&&"string"===typeof a.animation&&(e.animation=google.maps.Animation[a.animation.toUpperCase()]);d=new google.maps.Marker(e);this._markers.push(d);if(f(d,"position")&&(d.getPosition().lat()&&d.getPosition().lng()&&this.bounds.extend(e.position),!0===this.options.markerFitBounds&&p===this.options.marker.length))return b.fitBounds(this.bounds);!0===this.options.markerCluster&&"function"===typeof MarkerClusterer&&
p===this.options.marker.length?this.markerCluster=new MarkerClusterer(b,this._markers):(c={map:b,css:g!==a.css?a.css:"",id:c},"string"===typeof a.label&&0!==a.label.length&&(c.text=a.label,c=new n(c),c.bindTo("position",d,"position"),c.bindTo("text",d,"position"),c.bindTo("visible",d),this._labels.push(c)),this.bindEvents(d,a.event))},markerByGeocoder:function(b,a){var d=new google.maps.Geocoder,c=this;-1!==a.addr.indexOf(",")&&(a.addr="loc: "+a.addr);d.geocode({address:a.addr},function(d,g){if(g===
google.maps.GeocoderStatus.OVER_QUERY_LIMIT)r.setTimeout(function(){c.markerByGeocoder(a)},c.interval);else if(g===google.maps.GeocoderStatus.OK){var h={},k={},k={},h=f(a,"title")?a.title.toString().replace(/<([^>]+)>/g,""):"",k=f(a,"text")?a.text.toString():"",k={map:b,position:d[0].geometry.location,title:h,infoWindow:new google.maps.InfoWindow({content:k}),animation:null},l=c.markerIcon(a);h&&(k.title=h);q+=1;if("string"===typeof l||f(l,"url"))k.icon=l;f(a,"animation")&&"string"===typeof a.animation&&
(k.animation=google.maps.Animation[a.animation.toUpperCase()]);h=new google.maps.Marker(k);c._markers.push(h);f(h,"position")&&h.getPosition().lat()&&h.getPosition().lng()&&c.bounds.extend(k.position);if(!0===c.options.markerFitBounds&&q===c.options.marker.length)return b.fitBounds(c.bounds);f(c.options,"markerCluster")&&"function"===typeof MarkerClusterer&&q===c.options.marker.length?c.markerCluster=new MarkerClusterer(b,c._markers):(k={map:c.map,css:a.css||""},"string"===typeof a.label&&0!==a.label.length&&
(k.text=a.label,k=new n(k),k.bindTo("position",h,"position"),k.bindTo("text",h,"position"),k.bindTo("visible",h),c._labels.push(k)),c.bindEvents(h,a.event))}})},directionService:function(b){var a=[],d=new google.maps.DirectionsService,c=new google.maps.DirectionsRenderer,e={travelMode:google.maps.DirectionsTravelMode.DRIVING,optimizeWaypoints:b.optimize||!1},f={},h=0,k=0;"string"===typeof b.from&&(e.origin=b.from);"string"===typeof b.to&&(e.destination=b.to);"string"===typeof b.travel&&b.travel.length&&
(e.travelMode=google.maps.TravelMode[b.travel.toUpperCase()]);f=l(g!==b.panel?b.panel:null);if(g!==b.waypoint&&0!==b.waypoint){h=0;for(k=b.waypoint.length;h<k;h+=1)a.push({location:b.waypoint[h].toString(),stopover:!0});e.waypoints=a}g!==e.origin&&g!==e.destination&&(d.route(e,function(a,b){b===google.maps.DirectionsStatus.OK&&c.setDirections(a)}),c.setMap(this.map),f.length&&c.setPanel(f.get(0)),this._directions.push(c))},bindEvents:function(b,a){var d=this,c={};switch(typeof a){case "function":google.maps.event.addListener(b,
